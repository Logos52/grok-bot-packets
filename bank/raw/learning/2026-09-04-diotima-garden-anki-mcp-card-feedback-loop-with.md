---
id: 2026-09-04-diotima-garden-anki-mcp-card-feedback-loop-with
kind: article
title: anki-mcp — card feedback loop with human-gate diffs
source: "https://github.com/diotima-garden/anki-mcp"
author: diotima-garden
...
| `add-cards-to-deck` | internal (worker) | Generates cards from a resolved deck context + input and pushes them. Grove-blind — takes a deck name, context file, and input. Used by the monorepo's `/pipe:add-cards-to-grove` pipeline. |
| `generate-cards` | internal (fork) | Pure generator — turns deck context + note types + input into card blocks, no side effects. Forked by `add-cards-to-deck`. |
...
│   ├── add-cards-to-deck/              Internal worker — generates + pushes cards (pipeline-only)
│   └── generate-cards/                 Internal fork — pure card generator, no side effects
